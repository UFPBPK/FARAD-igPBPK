...
Htc         : 0.412    : Hematocrit                                             
QCC         : 8.700    : L/h/kg, Cardiac Output                                  
...
double CPlas_free   = APlas_free/VPlas;            // Concentration of unbound drug in the blood 
double CPlas_free1  = APlas_free1/VPlas;           // Concentration of unbound metabolites in the blood 
...
double CL1          = AL1/VL;                      // Concentration of metabolites in the tissue of liver 
double CVL          = CL/PL;                       // Concentration of parent drug in the capillary blood of liver (mmol/L)
double CVL1         = CL1/PL1;                     // Concentration of metabolites in the capillary blood of liver (mmol/L)
double CK           = AK/VK;                       // Concentration of parent drug in the tissue of kidney 
double CK1          = AK1/VK;                      // Concentration of metabolites in the tissue of kidney 
double CVK          = CK/PK;                       // Concentration of parent drug in the capillary blood of kidney (mmol/L)
double CVK1         = CK1/PK1;                     // Concentration of metabolites in the capillary blood of kidney (mmol/L)
double CM           = AM/VM;                       // Concentration of parent drug in the tissue of muscle (mmol/L)
double CM1          = AM1/VM;                      // Concentration of metabolites in the tissue of muscle (mmol/L)
double CVM          = CM/PM;                       // Concentration of parent drug in the capillary blood of muscle (mmol/L)
double CVM1         = CM1/PM1;                     // Concentration of metabolites in the capillary blood of muscle (mmol/L)
...
double CRest        = ARest/VRest;                 // Drug concentration in the rest of the body (mmol/L)
double CVRest       = CRest/PRest;                 // Concentration of parent drug in the capillary blood of the rest of body
double CRest1       = ARest1/VRest1;               // Metabolites concentration in the rest of the body (mmol/L)
double CVRest1      = CRest1/PRest1;               // Metabolites concentration in the capillary blood of the rest of body
double CV           = (QL*CVL + QK*CVK + QF*CVF + QM*CVM + QRest*CVRest)/QC; // CV, the drug concentration in the blood (mmol/L)
double CV1          = (QL*CVL1 + QK*CVK1 + QM*CVM1 + QRest1*CVRest1)/QC;     // CV1, the metabolites concentration in the blood (mmol/L)
...
double RL           = QL*(CPlas - CVL)*Free - Rmet - Rmet1-Rbile + Kabs*ASI + Kehc*ASI; 
...
//-- The chaning rate of the amount of dose  
double RST          = -GE*AST;                                                             
double RSI          = Rbile + Rbile1 + GE*AST - Kabs*ASI - Kehc*ASI - Kunabs*ASI;                                               
...
double RK           = QK*(CPlas - CVK)*Free - Rurine; 
...
// Muscle compartment, flow-limited model
...
// Fat compartment, flow-limited model
double RF           = QF*(CPlas - CVF)*Free; 
dxdt_AF             = RF; 

// Compartment of rest of body, flow-limited model
