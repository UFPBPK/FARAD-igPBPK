##---------------------------------------------------------------------------------------------------
# This function generates a list of the partition coefficients based on the Poulin and Theil method
...
## of the Poulin and Theil: http://jpharmsci.org/article/S0022-3549(16)30889-9/fulltext
...
Kp_PT <- function (logP, pKa, fup, BP = 1, type = 1, dat) {
    ...
    ## Estiamte the the D_star: "olive oil:buffer (water) partition coefficient" of both the--
    ...
    ## Henderson-Hasselbalch Equations are based on the Pulin and Theil, 2001: DOI: 10.1002/1520-6017(200104)90:4<436::aid-jps1002>3.0.co;2-p 
    ...
    D_star <- 10^logD_star
    Kp_ad <- ((D_star*(Vnlad+0.3*Vphad)+(1*(Vwad+0.7*Vphad)))/(D_star*(Vnlp+0.3*Vphp)+(1*(Vwp+0.7*Vphp)))) * fup
    ...
    Kp_tis <- ((P*(Vnlt+0.3*Vpht)+(1*(Vwt+0.7*Vpht)))/(P*(Vnlp+0.3*Vphp)+(1*(Vwp+0.7*Vphp)))) * (fup/fut)
