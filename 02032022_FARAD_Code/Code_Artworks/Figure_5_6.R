...
## Cattle
Sim_C_A_PG <-pred (N=N, pars = Pars_C_PG, drug = "PG", idata = idata_C_PG, 
                   tinterval = 24, Dose = 6.5, 
                   Dtimes = 5, route = 'im')

Sim_C_B_PG <-pred (N=N,pars = Pars_C_PG,drug = "PG", idata = idata_C_PG, 
                   tinterval = 24, Dose = 32.5, 
                   Dtimes = 5, route = 'im')

## Swine
Sim_S_A_PG <-pred (N=N,pars = Pars_S_PG, drug = "PG", idata = idata_S_PG, 
                   tinterval = 24, Dose = 6.5, 
                   Dtimes = 5, route = 'im')

Sim_S_B_PG <-pred (N=N,pars = Pars_S_PG,drug = "PG", idata = idata_S_PG, 
                   tinterval = 24, Dose = 32.5, 
                   Dtimes = 5, route = 'im')
...
WDIs_PG_C_L_A <- PG_WDI_C_A %>% filter(round(L99,3) <= 0.05 & Time > 24*4)%>%mutate(WDIs = (Time/24)-4)%>%select(WDIs)%>%min()
WDIs_PG_C_K_A <- PG_WDI_C_A %>% filter(round(K99,3) <= 0.05 & Time > 24*4)%>%mutate(WDIs = (Time/24)-4)%>%select(WDIs)%>%min()
WDIs_PG_C_M_A <- PG_WDI_C_A %>% filter(round(M99,3) <= 0.05 & Time > 24*4)%>%mutate(WDIs = (Time/24)-4)%>%select(WDIs)%>%min()

WDIs_PG_C_L_B <- PG_WDI_C_B %>% filter(round(L99,3) <= 0.05 & Time > 24*4)%>%mutate(WDIs = (Time/24)-4)%>%select(WDIs)%>%min()
WDIs_PG_C_K_B <- PG_WDI_C_B %>% filter(round(K99,3) <= 0.05 & Time > 24*4)%>%mutate(WDIs = (Time/24)-4)%>%select(WDIs)%>%min()
WDIs_PG_C_M_B <- PG_WDI_C_B %>% filter(round(M99,3) <= 0.05 & Time > 24*4)%>%mutate(WDIs = (Time/24)-4)%>%select(WDIs)%>%min()

## Tolerance of PG 25 ppb (0.025 ug/g) in Edible Tissue in swine
WDIs_PG_S_L_A <- PG_WDI_S_A %>% filter(round(L99,3) <= 0.025 & Time > 24*4)%>%mutate(WDIs = (Time/24)-4)%>%select(WDIs)%>%min()
WDIs_PG_S_K_A <- PG_WDI_S_A %>% filter(round(K99,3) <= 0.025 & Time > 24*4)%>%mutate(WDIs = (Time/24)-4)%>%select(WDIs)%>%min()
WDIs_PG_S_M_A <- PG_WDI_S_A %>% filter(round(M99,3) <= 0.025 & Time > 24*4)%>%mutate(WDIs = (Time/24)-4)%>%select(WDIs)%>%min()

WDIs_PG_S_L_B <- PG_WDI_S_B %>% filter(round(L99,3) <= 0.025 & Time > 24*4)%>%mutate(WDIs = (Time/24)-4)%>%select(WDIs)%>%min()
WDIs_PG_S_K_B <- PG_WDI_S_B %>% filter(round(K99,3) <= 0.025 & Time > 24*4)%>%mutate(WDIs = (Time/24)-4)%>%select(WDIs)%>%min()
WDIs_PG_S_M_B <- PG_WDI_S_B %>% filter(round(M99,3) <= 0.025 & Time > 24*4)%>%mutate(WDIs = (Time/24)-4)%>%select(WDIs)%>%min()
...
## Scenario A (Label dose) 2.2 mg/kg x 3 via IV injection
## Scenario B (Extra label) 2.2 mg/kg x 3 via IM injection
WDIs_FLU_C_L_A <- FLU_WDI_C_A %>% filter(round(L99,3) <= 0.125 & Time >24*2)%>%mutate(WDIs = (Time/24)-2)%>%select(WDIs)%>%min()
WDIs_FLU_C_K_A <- FLU_WDI_C_A %>% filter(round(K99,3) <= 0.125 & Time >24*2)%>%mutate(WDIs = (Time/24)-2)%>%select(WDIs)%>%min()
WDIs_FLU_C_M_A <- FLU_WDI_C_A %>% filter(round(M99,3) <= 0.025 & Time >24*2)%>%mutate(WDIs = (Time/24)-2)%>%select(WDIs)%>%min()

WDIs_FLU_C_L_B <- FLU_WDI_C_B %>% filter(round(L99,3) <= 0.125 & Time >24*2)%>%mutate(WDIs = (Time/24)-2)%>%select(WDIs)%>%min()
WDIs_FLU_C_K_B <- FLU_WDI_C_B %>% filter(round(K99,3) <= 0.125 & Time >24*2)%>%mutate(WDIs = (Time/24)-2)%>%select(WDIs)%>%min()
WDIs_FLU_C_M_B <- FLU_WDI_C_B %>% filter(round(M99,3) <= 0.025 & Time >24*2)%>%mutate(WDIs = (Time/24)-2)%>%select(WDIs)%>%min()
...
WDIs_FLU_S_L_A <- FLU_WDI_S_A %>% filter(round(L99,3) <= 0.030& Time >24*1)%>%mutate(WDIs = (Time/24)-0)%>%select(WDIs)%>%min()
WDIs_FLU_S_K_A <- FLU_WDI_S_A %>% filter(round(K99,3) <= 0.030& Time >24*1)%>%mutate(WDIs = (Time/24)-0)%>%select(WDIs)%>%min()
WDIs_FLU_S_M_A <- FLU_WDI_S_A %>% filter(round(M99,3) <= 0.025& Time >24*1)%>%mutate(WDIs = (Time/24)-0)%>%select(WDIs)%>%min()

WDIs_FLU_S_L_B <- FLU_WDI_S_B %>% filter(round(L99,3) <= 0.030& Time >24*3)%>%mutate(WDIs = (Time/24)-2)%>%select(WDIs)%>%min()
WDIs_FLU_S_K_B <- FLU_WDI_S_B %>% filter(round(K99,3) <= 0.030& Time >24*3)%>%mutate(WDIs = (Time/24)-2)%>%select(WDIs)%>%min()
WDIs_FLU_S_M_B <- FLU_WDI_S_B %>% filter(round(M99,3) <= 0.025& Time >24*3)%>%mutate(WDIs = (Time/24)-2)%>%select(WDIs)%>%min()
...
WDIs_FLO_C_L_A <- FLO_WDI_C_A %>% filter(round(L99,3) <= 3.7 & Time >24*2)%>%mutate(WDIs = (Time/24)-2)%>%select(WDIs)%>%min()
WDIs_FLO_C_K_A <- FLO_WDI_C_A %>% filter(round(K99,3) <= 3.7 & Time >24*2)%>%mutate(WDIs = (Time/24)-2)%>%select(WDIs)%>%min()
WDIs_FLO_C_M_A <- FLO_WDI_C_A %>% filter(round(M99,3) <= 0.3 & Time >24*2)%>%mutate(WDIs = (Time/24)-2)%>%select(WDIs)%>%min()

WDIs_FLO_C_L_B <- FLO_WDI_C_B %>% filter(round(L99,3) <= 3.7 & Time >24*1)%>%mutate(WDIs = (Time/24)-0)%>%select(WDIs)%>%min()
WDIs_FLO_C_K_B <- FLO_WDI_C_B %>% filter(round(K99,3) <= 3.7 & Time >24*1)%>%mutate(WDIs = (Time/24)-0)%>%select(WDIs)%>%min()
WDIs_FLO_C_M_B <- FLO_WDI_C_B %>% filter(round(M99,3) <= 0.3 & Time >24*1)%>%mutate(WDIs = (Time/24)-0)%>%select(WDIs)%>%min()

## Scenario C (Extra-label use): 20 mg/kg x 3 via IM injection (48 hrs interval)
## Scenario D (Extra-label use): 40 mg/kg x 3 via SC injection (96 hrs interval)
WDIs_FLO_C_L_C <- FLO_WDI_C_C %>% filter(round(L99,3) <= 3.7 & Time >48*2)%>%mutate(WDIs = (Time/24)-4)%>%select(WDIs)%>%min()
WDIs_FLO_C_K_C <- FLO_WDI_C_C %>% filter(round(K99,3) <= 3.7 & Time >48*2)%>%mutate(WDIs = (Time/24)-4)%>%select(WDIs)%>%min()
WDIs_FLO_C_M_C <- FLO_WDI_C_C %>% filter(round(M99,3) <= 0.3 & Time >48*2)%>%mutate(WDIs = (Time/24)-4)%>%select(WDIs)%>%min()

WDIs_FLO_C_L_D <- FLO_WDI_C_D %>% filter(round(L99,3) <= 3.7 & Time >96*2)%>%mutate(WDIs = (Time/24)-8)%>%select(WDIs)%>%min()
WDIs_FLO_C_K_D <- FLO_WDI_C_D %>% filter(round(K99,3) <= 3.7 & Time >96*2)%>%mutate(WDIs = (Time/24)-8)%>%select(WDIs)%>%min()
WDIs_FLO_C_M_D <- FLO_WDI_C_D %>% filter(round(M99,3) <= 0.3 & Time >96*2)%>%mutate(WDIs = (Time/24)-8)%>%select(WDIs)%>%min()

## Scenario A (label use): 30 mg/kg x 5 via PO (24 hrs interval)
## 100 mg/l (in water) * 10 l/day (water consumption from Thacker (2001)) = 1000 mg/day
## 1000 mg/day / 70 kg (swine body weight) = 14 mg/kg/day

WDIs_FLO_S_L_A <- FLO_WDI_S_A %>% filter(round(L99,3) <= 2.5 & Time >24*4)%>%mutate(WDIs = (Time/24)-4)%>%select(WDIs)%>%min()
WDIs_FLO_S_K_A <- FLO_WDI_S_A %>% filter(round(K99,3) <= 2.5 & Time >24*4)%>%mutate(WDIs = (Time/24)-4)%>%select(WDIs)%>%min()
WDIs_FLO_S_M_A <- FLO_WDI_S_A %>% filter(round(M99,3) <= 0.2 & Time >24*4)%>%mutate(WDIs = (Time/24)-4)%>%select(WDIs)%>%min()

WDIs_FLO_S_L_B <- FLO_WDI_S_B %>% filter(round(L99,3) <= 2.5 & Time >96*1)%>%mutate(WDIs = (Time/24)-4)%>%select(WDIs)%>%min()
WDIs_FLO_S_K_B <- FLO_WDI_S_B %>% filter(round(K99,3) <= 2.5 & Time >96*1)%>%mutate(WDIs = (Time/24)-4)%>%select(WDIs)%>%min()
WDIs_FLO_S_M_B <- FLO_WDI_S_B %>% filter(round(M99,3) <= 0.2 & Time >96*1)%>%mutate(WDIs = (Time/24)-4)%>%select(WDIs)%>%min()
